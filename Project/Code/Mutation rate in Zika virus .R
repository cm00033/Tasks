...
sample
model<-lm(RandomMutation~sample)
model
length(RandomMutation)
length(sample)
dim(RandomMutation)

par(mfrow=c(1,2), mar=c(4,4,1,1))
plot(sample,RandomMutation, col='green',pch=16,cex=1.5,xlab="Population", ylab="Random Mutation Rate")
title(main="Random Mutation vs Population size")
abline(model,col='black')
PearTest<-cor.test(RandomMutation,sample)
PearTest

ANOVA<-aov(RandomMutation~sample)
ANOVA